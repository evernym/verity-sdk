...
| 'did:sov:d8xBkXpPgvyR=d=xUzi42=PBbw;spec/write-cred-def/0.1/write'
| 'did:sov:d8xBkXpPgvyR=d=xUzi42=PBbw;spec/write-cred-def/0.1/problem-report'
| 'did:sov:d8xBkXpPgvyR=d=xUzi42=PBbw;spec/write-cred-def/0.1/status'
...
export class CredentialDef extends Protocol {
    ...
    public router(message: ICredWriteMessage) {
        switch (message['@type']) {
            case 'did:sov:d8xBkXpPgvyR=d=xUzi42=PBbw;spec/write-cred-def/0.1/write':
    ...
    private generateStatusReport(status: number, statusMessage: string, message: ICredWriteMessage, content?: any) {
        let msg = {
            '@type': 'did:sov:d8xBkXpPgvyR=d=xUzi42=PBbw;spec/write-cred-def/0.1/status',
            '@id': uuid(),
            '~thread': {
                pthid: message['@id'],
                seqnum: 0,
