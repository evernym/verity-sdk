...
async def example():
    ...
    def print_invite_details(msg: dict) -> None:
    ...
    @AddHandler(handlers, message_type=Connecting.get_status_message_type(), message_status=Connecting.INVITE_ACCEPTED_STATUS)
    ...
    @AddHandler(handlers, message_type=QuestionAnswer.get_status_message_type(), message_status=QuestionAnswer.QUESTION_ANSWERED_STATUS)
    ...
    @AddHandler(handlers, message_type=WriteSchema.get_status_message_type(), message_status=WriteSchema.WRITE_SUCCESSFUL_STATUS)
    async def write_cred_def_to_ledger(msg: dict) -> None:
        write_cred_def = WriteCredentialDefinition(name="Test Credential Definition", schema_id=msg['content'], tag="latest", revocation_details={'support_revocation': False})
        await write_cred_def.write(context)

    @AddHandler(handlers, message_type=WriteCredentialDefinition.get_status_message_type(), message_status=WriteCredentialDefinition.WRITE_SUCCESSFUL_STATUS)
    ...
    @AddHandler(handlers, message_type=IssueCredential.get_status_message_type(), message_status=IssueCredential.OFFER_ACCEPTED_BY_USER_STATUS)
    def print_credential_status(msg: dict) -> None:
        print("User has accepted the credential offer. Verity is now sending the Credential")

    @AddHandler(handlers, message_type=IssueCredential.get_status_message_type(), message_status=IssueCredential.CREDENTIAL_SENT_TO_USER_STATUS)
    ...
    @AddHandler(handlers, message_type=PresentProof.get_status_message_type(), message_status=PresentProof.PROOF_RECEIVED_STATUS)
    def print_proof(msg: dict) -> None:
    ...
    def problem_report_handler(msg: dict) -> None:
