...
namespace VeritySDK
{
    ...
    public class WalletUtil
    {
        ...
        private static void tryToCreateWallet(string walletConfig, string walletCredentials) // throws WalletException
        {
            try
            {
                //if (Sdk.LibIIndy.api == null)
                //{
                //    throw new WalletException("Libindy failed to initialize - likely the shared library was not found");
                //}
                Hyperledger.Indy.WalletApi.Wallet.CreateWalletAsync(walletConfig, walletCredentials).GetAwaiter();
            }
        ...
        public static Hyperledger.Indy.WalletApi.Wallet openIndyWallet(WalletConfig walletConfig) // throws WalletOpenException, JSONException 
        {
            try
            {
                //if (LibIndy.api == null)
                ...
                //}
                return Hyperledger.Indy.WalletApi.Wallet.OpenWalletAsync(walletConfig.config(), walletConfig.credential()).GetAwaiter().GetResult();
            }
        ...
        public static void deleteWallet(string walletName, string walletKey) // throws WalletException
        {
            ...
            try
            {
                ...
                //    throw new WalletException("Libindy failed to initialize - likely the shared library was not found");
                //}
                Hyperledger.Indy.WalletApi.Wallet.DeleteWalletAsync(walletConfig, walletCredentials).GetAwaiter();
            }
            catch (IndyException /*| InterruptedException | ExecutionException*/ e)
            {
                if (!(e.InnerException != null && e.InnerException is WalletExistsException))
