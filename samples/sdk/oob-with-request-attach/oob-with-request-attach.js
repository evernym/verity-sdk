...
// Maps containing unresolved promises. ThreadId is used as the map key
...
// Initialize function is run at application start up
// It will create Issuer context, Verifier context, schema and credential definition if it is the first time run
// or will reload contexts and cred_def_id from files if it is application re-run
...
// Provisions Issuer
// Create Issuer context, update webhook, setup Issuer DID/Verkey and write them to the ledger
// create schema and credential definition needed for credential issuance
...
async function setupIssuer () {
  ...
  const spinner = new Spinner('Waiting for setup Issuer to complete ... %s').setSpinnerDelay(450)

  const setupIssuerPromise =
  ...
  await issuerSetup.create(issuerContext)
  spinner.stop()
  await setupIssuerPromise // wait for request to complete
}
...
async function writeLedgerSchema () {
  ...
  const spinner = new Spinner('Waiting to write schema to ledger ... %s').setSpinnerDelay(450)
  ...
  await schemaCreate.write(issuerContext) // wait for operation to be complete
  spinner.stop()
  return schemaPromise // returns ledger schema identifier
}
...
async function writeLedgerCredDef (schemaId) {
  ...
  const spinner = new Spinner('Waiting to write cred def to ledger ... %s').setSpinnerDelay(450)
  ...
  await credDefCreate.write(issuerContext) // wait for operation to be complete and returns ledger cred def identifier
  spinner.stop()
...
// Provisions Verifier
// Create Verifier context, update webhook and save it to a file
...
async function createRelationship (role) {
  ...
  const spinner = new Spinner('Waiting to create relationship ... %s').setSpinnerDelay(450)
  ...
  const relDID = await relationshipCreate // wait for response from verity application
  spinner.stop()
  //   const relDID = relationshipKeys[0]
  // const threadId = relProvisioning.threadId

  // spinner = new Spinner('Waiting to create invitation ... %s').setSpinnerDelay(450)
  ...
  //           console.log(`Invite URL: ${message.inviteURL}`)
  //           resolve(null)
  //           break
  //         default:
  //           printMessage(msgName, message)
  ...
  //   await relationship.outOfBandInvitation(issuerContext, true)
  //   spinner.stop()
...
// Registers handlers for messages received from VAS
function registerHandlers () {
    ...
    handlers.addHandler(issuerSetup.msgFamily, issuerSetup.msgFamilyVersion, async (msgName, message) => {
      switch (msgName) {
        case issuerSetup.msgNames.PUBLIC_IDENTIFIER_CREATED: {
          ...
          issuerSetupMap.get(threadId)(null)
        }
          break
        default:
          printMessage(msgName, message)
    ...
    const schema = new sdk.protocols.WriteSchema()
    ...
    // add handler for relationship create
    const relProvisioning = new sdk.protocols.v1_0.Relationship()
    handlers.addHandler(relProvisioning.msgFamily, relProvisioning.msgFamilyVersion, async (msgName, message) => {
      const threadId = message['~thread'].thid
      switch (msgName) {
    ...
    // Connecting protocol is initiated from ConnectMe side
...
async function main () {
  ...
  app.post('/issueCredential', async (req, res) => {
    ...
    const spinner = new Spinner('Create invitation with credential offer attached... %s').setSpinnerDelay(450)

    // Promise for 'protocol-invitation' message expected after offer credential is sent
    ...
    // wait till the user scans QR code and accepts credentials
    const credentialSentPromise = new Promise((resolve) => {
      issueCredSentMap.set(issue.threadId, resolve)
    })

    await credentialSentPromise
    // At this time F/E could be notified that credential has been issued
    // e.g. A notification to F/E could be sent via IO sockets
    // Alternatively, polling for credential status could be implemented on F/E
  })
  ...
  app.post('/requestProof', async (req, res) => {
    ...
    const spinner = new Spinner('Create invitation with proof request attached ... %s').setSpinnerDelay(450)

    /// Promise for 'protocol-invitation' message expected when proof request is sent
    const protocolInvitationPromise = new Promise((resolve) => {
    ...
    // At this time F/E could be notified with verification results
    // e.g. A notification to F/E could be sent via IO sockets
    // Alternatively, polling for proof status could be implemented on F/E
  })
