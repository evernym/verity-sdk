...
async function createConnection () {
  ...
  const relationshipCreate =
    new Promise(function (resolve, reject) {
      relCreateResolveMap[relThreadId] = resolve
  ...
  const relationshipInvitation =
    new Promise(function (resolve, reject) {
      relInvitationResolveMap[relThreadId] = resolve
  ...
  InviteToDidMap[invitationId] = relationshipDid
  ...
  const ConnectionPromise =
    new Promise(function (resolve, reject) {
      connectionResolveMap[relationshipDid] = resolve
    })
...
app.post('/webhook', async (req, res) => {
  ...
  switch (message['@type']) {
    ...
    case 'did:sov:123456789abcdefghi1234;spec/relationship/1.0/created':
      ...
      relCreateResolveMap[threadId](message.did)
      break
    case 'did:sov:123456789abcdefghi1234;spec/relationship/1.0/invitation':
      ...
      relInvitationResolveMap[threadId]([message.inviteURL, message.invitationId])
      break
    ...
    case 'did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/connections/1.0/response-sent':
      ...
      connectionResolveMap[message.myDID](['accepted', null])
      break
    ...
    case 'did:sov:BzCbsNYhMrjHiqZDTUASHg;spec/out-of-band/1.0/relationship-reused':
      ...
      // Resolve connection promise with the status ('accepted' or 'redirected') and pthid
      connectionResolveMap[InviteToDidMap[pthid]](['redirected', message.relationship])
      break
    default:
