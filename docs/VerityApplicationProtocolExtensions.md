...
### JSON and DID Comms
Because of the flexibility of JSON it is expected that JSON formatting for interaction with the Verity server will follow [this](https://docs.google.com/document/d/1flfXrvZ0ehzB6v69VjVLJXFyITCLjla58d7SNYVNhac/edit) document and its guidelines around DID Comm messages.

Message attributes and structure for all Verity protocol extensions will adhere to this [HIPE](https://github.com/hyperledger/indy-hipe/tree/master/text/0021-message-types#message-type-design-guidelines) to ensure attributes are consistent and readable.
...
Due to the asynchronous nature of the Verity service and its protocols, messages are broken up into two general types: *request messages* and *response messages*, which is based on the state machine pattern for any request/response interaction defined [here](https://github.com/hyperledger/indy-hipe/blob/9c7722d208cfe0a336cb67a626cbbb192ae73f8c/text/feature-discovery/README.md#states).
...
This protocol defines general Verity service message types. This protocol is intended to provide convenience functionality that may be used across different protocols as well as messages that can be passed for any reason, such as error handling. For example with the `check-status` message, you may want to use it to get a status of an onboard request, and in parallel get the status of a separate credential issuance, which are two very different protocol families, but can be queried using the same common protocol.
...
Problem report for non-specific errors. Generally, problem reports will be handled by specific protocols (connection, cred, enroll), but sometimes a more generalized problem report needs to be generated, such as: the service had a general failure, insufficient resources, or if a bad message was passed to the Verity service. Because these reports encompass potentially a lot of detail and use cases, and error handling is rather difficult by nature, it is recommended that you read [this hipe](https://github.com/hyperledger/indy-hipe/blob/d6503aebbc03b80796c6801ade9265592f9d5521/text/error-reporting/README.md#00-error-reporting) on error reporting, which the Verity service models for error-handling patterns, and will greatly help in building solid error handling to respond to these messages. Keep in mind that the Verity service will not likely use every attribute described in the HIPE, but will attempt to use the key attributes that describe the error adequately.
...
* `comment` field is **required**, but does not have to contain every element below, and yet needs **>1** of the attributes below. Details the error, and contains: 
...
The *Connection* set of protocols encompasses the necessary functionality to allow an enterprise to create a pairwise connection with an identity owner.
...
The *Provable Question* set of protocols encompasses the necessary functionality to allow an enterprise to ask questions through a pairwise connection and receive a signed response.
...
The *Schema* set of protocols encompasses the necessary functionality to allow an enterprise to write a schema to the ledger.
...
The *Credential Definition* set of protocols encompasses the necessary functionality to allow an enterprise to write a credential to the ledger.
...
The *Credential* set of protocols encompasses the necessary functionality to allow an enterprise to issue a credential to an existing connection.
...
* `credentialData`
	...
	* `name` name of the credential. e.g. Driver's License
...
The *Proof* set of protocols encompasses the necessary functionality to allow an enterprise to send a proof request to, and receive a proof from, an existing connection.
...
The *Enroll* set of protocols encompasses the necessary functionality to allow an enterprise to create a pairwise connection with an identity owner and quickly issue a specific credential.
